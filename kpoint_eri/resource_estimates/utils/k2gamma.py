...
Slightly modified to stop failures for complex orbitals / assertion errors for
metals.
...
def mo_k2gamma(cell, mo_energy, mo_coeff, kpts, kmesh=None, make_real=True):
    ...
    try:
        delta_ovlp = abs(
            reduce(np.dot, (C_gamma.conj().T, s, C_gamma)) - np.eye(Nmo * Nk)
        ).max()
    ...
    if make_real:
        if np.any(E_k_degen):
            if abs(C_gamma[:, ~degen_mask].imag).max() < 1e-4:
                ...
                f = np.dot(
                    C_gamma[:, degen_mask] * (E_g[degen_mask] - shift),
...
def k2gamma(kmf, kmesh=None, make_real=True):
    ...
    def transform(mo_energy, mo_coeff, mo_occ):
        scell, E_g, C_gamma = mo_k2gamma(
            kmf.cell, mo_energy, mo_coeff, kmf.kpts, kmesh, make_real=make_real
        )[:3]
    ...
    if isinstance(kmf, scf.khf.KRHF):
        ...
        scell, Ea, Ca, occ_a = transform(
            kmf.mo_energy[0], kmf.mo_coeff[0], kmf.mo_occ[0]
        )
        scell, Eb, Cb, occ_b = transform(
            kmf.mo_energy[1], kmf.mo_coeff[1], kmf.mo_occ[1]
        )
        if isinstance(kmf.with_df, RSDF):
