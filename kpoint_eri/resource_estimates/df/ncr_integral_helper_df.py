...
class DFABKpointIntegrals:
    ...
    def build_AB_from_chol(self, qidx):
        """
        Construct A_{n}(\Q) and B_{n}(Q) matrices given Q-kpt idx.  This constructs
...
class DFABV2KpointIntegrals:
    ...
    def build_chol_part_from_A_B(self, 
                                 kidx: int, 
                                 qidx: int,
                                 Amats: np.ndarray, 
                                 Bmats: np.ndarray,
    ...
    def get_eri(self, ikpts):
        ...
        # build Cholesky vector from truncated A and B
        chol_val_k_kmq = self.build_chol_part_from_A_B(ikp, 
                                                       qidx, 
                                                       self.amat_n_mats[ikp, qidx], self.bmat_n_mats[ikp, qidx])
        chol_val_kp_kpmq = self.build_chol_part_from_A_B(iks, 
                                                         qidx, 
                                                         self.amat_n_mats[iks, qidx], self.bmat_n_mats[iks, qidx])
