...
def kpoint_single_factorization_costs(n: int, 
    ...
    for p in range(20):
        # JJG note: arccos arg may be > 1
        # v = Round[2^p/(2*\[Pi])*ArcCos[2^nL/Sqrt[(L + 1)/2^\[Eta]]/2]];
        v = np.round(np.power(2,p+1) / (2 * np.pi) * arccos(np.power(2,nL) / np.sqrt(L / np.power(2, eta)) / 2))
        #   oh[[p]] = stps*(1/N[Sin[3*ArcSin[Cos[v*2*\[Pi]/2^p]*Sqrt[(L + 1)/2^\[Eta]]/2^nL]]^2] - 1) + 4*p];
        # oh2[[p]] = stps*(1/N[Sin[3*ArcSin[Cos[v*2*\[Pi]/2^p]*Sqrt[L/2^\[Eta]]/2^nL]]^2] - 1) + 4*p];
        oh2[p] = np.real(stps * (1 / (np.sin(3 * arcsin(np.cos(v * 2 * np.pi / np.power(2,p+1)) * \
                                                       np.sqrt(L / np.power(2, eta)) / np.power(2,nL)
                                                       )
                                            )**2
                                    ) - 1
                                ) + 4 * (p + 1)
    ...
    cost = cost1a + cost1b + cost1cd + cost2a + cost2b + cost2cd + cost3 + cost4 + cost6 + cost7 + cost9 + cost10
