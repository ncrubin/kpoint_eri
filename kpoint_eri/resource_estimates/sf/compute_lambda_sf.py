...
def compute_lambda_ncr(hcore, sf_obj: NCRSingleFactorizationHelper):
    ...
    for qidx in range(len(kpts)):
        ...
        # A /= np.sqrt(nkpts)
        # B /= np.sqrt(nkpts)
        for nn in range(sf_obj.naux):
            sum_to_be_squared = 0
            for kidx in range(sf_obj.nk):
                k_minus_q_idx = sf_obj.k_transfer_map[qidx, kidx]
                sum_to_be_squared += np.sum(np.abs(sf_obj.chol[kidx, k_minus_q_idx][nn, :, :].real / np.sqrt(nkpts)))
                sum_to_be_squared += np.sum(np.abs(sf_obj.chol[kidx, k_minus_q_idx][nn, :, :].imag / np.sqrt(nkpts)))
            lambda_two_body += sum_to_be_squared**2
    lambda_two_body *= 0.5

    lambda_tot = lambda_one_body + lambda_two_body
    return lambda_tot, lambda_one_body, lambda_two_body

def compute_lambda_ncr2(hcore, sf_obj: NCRSingleFactorizationHelper):
    """
