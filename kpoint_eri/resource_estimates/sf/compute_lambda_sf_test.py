...
def test_lambda_calc():
    ...
    supercell_helper = SingleFactorizationHelper(
    ...
    lambda_data_sc = compute_lambda(supercell_hcore_mo, supercell_helper)

    # check l2 norm of one-body
    assert np.isclose(
        np.linalg.norm(supercell_hcore_mo), np.linalg.norm(np.array(hcore_mo).ravel())
    )
