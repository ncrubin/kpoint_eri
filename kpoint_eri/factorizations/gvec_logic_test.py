...
get_delta_kp_kq_Q, build_transfer_map, build_G_vectors, build_gpq_mapping, build_conjugate_map)
...
def test_transfer_map():
    ...
    for kpidx, kqidx in itertools.product(range(nkpts), repeat=2):
        ...
        q_scaled_kpt_aliased = np.array([q_scaled_kpt[0] % kmesh[0],
                                         q_scaled_kpt[1] % kmesh[1],
                                         q_scaled_kpt[2] % kmesh[2],
                                        ])
        qidx = np.where((scaled_kpts[:, 0] == q_scaled_kpt_aliased[0]) & (scaled_kpts[:, 1] == q_scaled_kpt_aliased[1]) & (scaled_kpts[:, 2] == q_scaled_kpt_aliased[2]) )[0]
...
def test_compliment_g():
    ...
    for qidx, kpidx, ksidx in itertools.product(range(nkpts), repeat=3):
        ...
        conserved_k = scaled_kpts[kpidx] - scaled_kpts[kqidx] + scaled_kpts[kridx] - scaled_kpts[ksidx]
